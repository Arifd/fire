export const fireFragShaderString = `
...
void main() {
  ...
  // just a basic gradient instead of the flame-y one above
  // float g = mix(0.0, -1.0, uv.y - 0.2);
  ...
  //float alpha = 0.0;
  //if ((ng + smoke) > 0.1) alpha = 1.0; else alpha = (ng + smoke);
  float alpha = sqrt(ng + smoke) * 1.25;
  gl_FragColor = vec4(vec3(colour), alpha);
}
`